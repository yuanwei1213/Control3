...
public class TakePicture {
    ...
    public void setmPictureComplate(PictureComplate mPictureComplate) {
        this.mPictureComplate = mPictureComplate;
    }

    public  PictureComplate mPictureComplate;
    public  interface  PictureComplate{

        void  complate(File file);
    }
...
//        initCamera();
    ...
    // 初始化摄像头
    public void initCamera(int state) {
        mState=state;
        // 如果存在摄像头
        if (checkCameraHardware(mContext.getApplicationContext())) {
            // 获取摄像头（首选前置，无前置选后置）
            //前置
            if (openFacingFrontCamera(state)) {
...
//                    .show();
            ;
//            Log.i(TAG, "onPictureTaken: "+mPictureComplate);
//            Log.i(TAG, "onPictureTaken: bitmap"+bitmap);
            mPictureComplate.complate(pictureFile);
            myCamera.stopPreview();
            myCamera.release();
            myCamera = null;
        }
    ...
    // 得到后置摄像头
    private boolean openFacingFrontCamera(int state) {
        // 尝试开启前置摄像头
